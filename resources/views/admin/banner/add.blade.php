...
<script>
                ...
                if (response.status == 200) {
                    let newBanner = $(`
                        <li class="list-group-item d-flex flex-column align-items-center justify-content-center position-relative" data-id="${response.data.id}">
                            ...
                            <!-- Action Buttons -->
                            <div class="banner-actions">
                                <button class="btn btn-primary btn-sm edit-banner" data-id="${response.data.id}">
                                    <em class="icon ni ni-edit"></em>
                                    <span>Edit</span>
                                </button>
                                <button class="btn btn-danger btn-sm delete-banner" data-id="${response.data.id}">
                                    <em class="icon ni ni-trash"></em>
                                    <span>Delete</span>
                                </button>
                            </div>
    ...
    $(document).on("click", ".edit-banner", function() {
        ...
        window.location.href = '{{ route( 'admin.banner.edit' ) }}?id=' + bannerId;
    });
