<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.developers' ) }}</h3>
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.developer.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'default',
        'id' => 'logo',
        'title' => __( 'developer.logo' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'developer.name' ) ] ),
        'id' => 'name',
        'title' => __( 'developer.name' ),
    ],
    [
        'type' => 'select',
        'options' => $data['developer_type'],
        'id' => 'developer_type',
        'title' => __( 'developer.developer_type' ),
    ],
    [
        'type' => 'select',
        'options' => $data['status'],
...
<x-data-tables id="developer_table" enableFilter="true" enableFooter="false" columns="{{ json_encode( $columns ) }}" />
...
var statusMapper = {
    ...
    typeMapper = @json( $data['developer_type'] ),
    dt_table,
    dt_table_name = '#developer_table',
    dt_table_config = {
        language: {
        ...
        ajax: {
            url: '{{ route( 'admin.developer.allDevelopers' ) }}',
            data: {
                '_token': '{{ csrf_token() }}',
            },
            dataSrc: 'developers',
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    if ( data ) {
                        ...
                        return '<img src="' + ( data ? data : '{{ asset( 'admin/logos/placeholder.png' ) }}' ) + '" width="75px" />';

                    } else {

                        return '<img src="' + '{{ asset( 'admin/logos/placeholder.png' ) }}' + '" width="75px" />'
            ...
            {
                targets: parseInt( '{{ Helper::columnIndex( $columns, "developer_type" ) }}' ),
                render: function( data, type, row, meta ) {
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit developers', 'delete developers' ] )
                    let edit, status = '';
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.developer.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.developer.updateDeveloperStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
