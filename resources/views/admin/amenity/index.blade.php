                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.amenity.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'default',
        'id' => 'icon',
        'title' => __( 'template.icon' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'template.title' ) ] ),
        'id' => 'title',
...
<x-data-tables id="amenity_table" enableFilter="true" enableFooter="false" columns="{{ json_encode( $columns ) }}" />
...
var statusMapper = {
    ...
    dt_table_config = {
        ...
        ajax: {
            url: '{{ route( 'admin.amenity.allAmenities' ) }}',
            data: {
                '_token': '{{ csrf_token() }}',
            },
            dataSrc: 'amenities',
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    if ( data ) {
                        return '<img src="' + data + '" width="50px" class="rounded" style="max-height: 50px; object-fit: cover;" />';
                    } else {
                        return '<span class="text-muted">{{ __( "template.no_icon" ) }}</span>';
                    }
                },
            },
            {
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit amenities', 'delete amenities' ] )
                    let edit, status = '';
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.amenity.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.amenity.updateAmenityStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
