        ...
        <div class="row gy-3">
            <div class="col-md-2">                
                <div class="list-group" role="tablist">
                    <a class="list-group-item list-group-item-action d-none " data-bs-toggle="list" href="#ms" role="tab">{{ __( 'setting.bonus_settings' ) }}</a>
            ...
            <div class="col-md-10">
                <div class="tab-content p-2">
                    <div class="tab-pane fade show" id="ms" role="tabpanel">
                        <h5 class="card-title mb-0">{{ __( 'setting.bonus_settings' ) }}</h5>
                        <hr>
                        <div class="row">
                            <div class="col-6">
                                <div class="mb-3 row">
                                    <label for="{{ $setting }}_whatsapp" class="col-sm-5 col-form-label">{{ __( 'setting.whatsapp' ) }} </label>
                                    <div class="col-sm-7">
                                        <input type="text" class="form-control form-control-sm" id="{{ $setting }}_whatsapp">
                                        <div class="invalid-feedback"></div>
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    <label for="{{ $setting }}_official_phone_number" class="col-sm-5 col-form-label">
                                        {{ __( 'setting.official_phone_number' ) }}
                                    </label>
                                    <div class="col-sm-7">
                                        <div class="input-group input-group-sm">
                                            <input type="text" 
                                                   class="form-control" 
                                                   id="{{ $setting }}_official_phone_number" 
                    ...
                    <div class="tab-pane fade show active" id="avs" role="tabpanel">
                        ...
                        <div class="row">
                            <div class="col-6">
                                <div class="mb-3 row">
                                    <label for="{{ $setting }}_app_version_apple" class="col-sm-5 col-form-label">{{ __( 'setting.app_version_apple' ) }}</label>
                                    <div class="col-sm-7">
                                        <input type="text" class="form-control form-control-sm" id="{{ $setting }}_app_version_apple">
                                        <div class="invalid-feedback"></div>
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    <label for="{{ $setting }}_app_version_playstore" class="col-sm-5 col-form-label">{{ __( 'setting.app_version_playstore' ) }}</label>
                                    <div class="col-sm-7">
                                        <input type="text" class="form-control form-control-sm" id="{{ $setting }}_app_version_playstore">
                                        <div class="invalid-feedback"></div>
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    <label for="{{ $setting }}_app_version_huawei" class="col-sm-5 col-form-label">{{ __( 'setting.app_version_huawei' ) }}</label>
                                    <div class="col-sm-7">
                                        <input type="text" class="form-control form-control-sm" id="{{ $setting }}_app_version_huawei">
                                        <div class="invalid-feedback"></div>
                                    </div>
                                </div>
                                <div class="text-end">
                                    <button class="btn btn-sm btn-primary" id="avs_save">{{ __( 'template.save_changes' ) }}</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( '#bs_save' ).on( 'click', function() {
            ...
            $.ajax( {
                ...
                data: {
                    ...
                    taxes: $( s + '_taxes' ).val(),
                    whatsapp: $( s + '_whatsapp' ).val(),
                    official_phone_number: $( s + '_official_phone_number' ).val(),
                    _token: '{{ csrf_token() }}',
                },
        ...
        $( '#avs_save' ).on( 'click', function() {
            ...
            $.ajax( {
                ...
                type: 'POST',
                data: {
                    app_version_apple: $( s + '_app_version_apple' ).val(),
                    app_version_playstore: $( s + '_app_version_playstore' ).val(),
                    app_version_huawei: $( s + '_app_version_huawei' ).val(),
                ...
                success: function( response ) {
                    if ( response ) {
                        response.forEach(item => {
                            ...
                            if (item.option_name === "WHATSAPP") {
                                $( s + '_whatsapp').val( item.option_value );
                            }

                            if (item.option_name === "OFFICIAL_PHONE_NUMBER") {
                                $( s + '_official_phone_number').val( item.option_value );
                            }

                            if (item.option_name === "APP_VERSION_APPLE") {
                                $( s + '_app_version_apple').val( item.option_value );
                            }

                            if (item.option_name === "APP_VERSION_PLAYSTORE") {
                                $( s + '_app_version_playstore').val( item.option_value );
                            }

                            if (item.option_name === "APP_VERSION_HUAWEI") {
                                $( s + '_app_version_huawei').val( item.option_value );
                            }
                        });
                    }
                },
