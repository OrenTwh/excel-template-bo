<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'Locations' ) }}</h3>
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.location.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'Code' ) ] ),
        'id' => 'code',
        'title' => __( 'Code' ),
    ],
    [
...
var statusMapper = @json( $data['status'] ),
    ...
    dt_table_config = {
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    return typeMapper[data] ? typeMapper[data] : data;
                },
            },
            {
                targets: parseInt( '{{ Helper::columnIndex( $columns, "full_path" ) }}' ),
                orderable: false,
                render: function( data, type, row, meta ) {
                    return data ? data : '-' ;
                },
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-status', function() {
            ...
            $.ajax( {
                url: '{{ route( 'admin.location.updateLocationStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
