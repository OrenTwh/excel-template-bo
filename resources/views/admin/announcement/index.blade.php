<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.announcements' ) }}</h3>
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.announcement.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
var statusMapper = {
    ...
    newUserOnlyStatusMapper = {
        '1': {
            'text': '{{ __( 'datatables.enabled' ) }}',
            'color': 'badge rounded-pill bg-success',
        },
        '0': {
            'text': '{{ __( 'datatables.disabled' ) }}',
            'color': 'badge rounded-pill bg-danger',
        },
    },
    ...
    dt_table_config = {
        ...
        columnDefs: [
            ...
            {
                targets: parseInt( '{{ Helper::columnIndex( $columns, "parent_announcement" ) }}' ),
                width: '10%',
                render: function( data, type, row, meta ) {
                    return data ? data : '-' ;
                },
            ...
            
            {
                ...
                width: '10%',
                render: function( data, type, row, meta ) {
                    return row.decoded_translations ? row.decoded_translations['en'].title : data ;
                },
            },
            {
            ...
            {
                targets: parseInt( '{{ Helper::columnIndex( $columns, "new_user_only" ) }}' ),
                render: function( data, type, row, meta ) {
                    return '<span class="' + newUserOnlyStatusMapper[data].color + '">' + newUserOnlyStatusMapper[data].text + '</span>';
                },
            },
