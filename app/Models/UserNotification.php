...
class UserNotification extends Model
{
    ...
    {
        return self::resolveTranslation($value);
    }
    ...
    private static function resolveTranslation($value): string
    {
        if (empty($value)) {
            return '';
        }
        ...
        // Not JSON — plain string stored directly
        ...
        $lang = trim((string) request()->input('lang', ''));

        if ($lang === '' || !array_key_exists($lang, $translations)) {
            $lang = 'en';
        }

        return (string) ($translations[$lang] ?? $translations['en'] ?? reset($translations) ?? '');
    }
