...
class VoucherService
{
    ...
    public static function createVoucher( $request ) {
        ...
        $validator = Validator::make( $request->all(), [
            ...
            'applicable_to' => [ 'nullable', 'in:all,booking,product' ],
        ...
        if ($adjustmentData && $request->discount_type == 2) {
    ...
    public static function updateVoucher( $request ) {
        ...
        try {
            ...
            $updateVoucher->applicable_to = $request->applicable_to ?? 'all';
    ...
    public static function getVouchers( $request )
    {
        ...
        if ( ! $request->user_voucher ) {
            ...
            $sorted = $vouchers->getCollection()->filter(function($voucher) {
                return $voucher->redeemable === true;
            });
    ...
    public static function validateVoucher( $request )
    {
        ...
        if ( $voucher->discount_type == 3 ) {
            ...
            $x = $cart->cartMetas->whereIn( 'ticket_type_id', $adjustment->buy_ticket_types )->count();

            if ( $x < $adjustment->buy_quantity ) {
                return response()->json( [
                   'required_amount' => $adjustment->buy_quantity,
                   'message' => __( 'voucher.min_quantity_of_x', [ 'title' => $adjustment->buy_quantity . ' ' . TicketType::where( 'id',  $adjustment->buy_ticket_types[0] )->value( 'name' ) ] ),
                   'message_key' => 'voucher.min_quantity_of_x_' . $adjustment->buy_ticket_types[0] . '_' .  TicketType::find( $adjustment->buy_ticket_types[0] )->value( 'name' ) ,
                        'errors' => [
                            'voucher' => __( 'voucher.min_quantity_of_x', [ 'title' => $adjustment->buy_quantity . ' ' . TicketType::where( 'id',  $adjustment->buy_ticket_types[0] )->value( 'name' ) ] )
            ...
            if ( $cart->total_price < $adjustment->buy_quantity ) {
                return response()->json( [
                    'required_amount' => $adjustment->buy_quantity,
                    'message' => __( 'voucher.min_spend_of_x', [ 'title' => $adjustment->buy_quantity . ' ' . TicketType::where( 'id', $adjustment->buy_ticket_types[0] )->value( 'name' ) ] ),
                    'message_key' => 'voucher.min_spend_of_x',
                    'errors' => [
                        'voucher' => __( 'voucher.min_spend_of_x', [ 'title' => $adjustment->buy_quantity . ' ' . TicketType::where( 'id', $adjustment->buy_ticket_types[0] )->value( 'name' ) ] )
                    ]
                ], 422 );
            }
    ...
    public static function giftVoucher( $request )
    {
        $authUser = auth()->user();

        $validator = Validator::make( $request->all(), [
            'user_voucher_id' => [ 'required' ],
            'friend_id'       => [ 'required' ],
        ] );
        $validator->validate();

        // Voucher must belong to the sender and be active
        ...
        if ( !$userVoucher ) {
            return response()->json( [
                'message' => __( 'voucher.voucher_not_available' ),
                'errors'  => [ 'user_voucher_id' => __( 'voucher.voucher_not_available' ) ],
            ], 422 );
        }

        // Recipient must be a friend
        $isFriend = UserFriend::where( 'status', UserFriend::STATUS_ACCEPTED )
            ->where( function( $q ) use ( $authUser, $request ) {
                $q->where( 'user_id', $authUser->id )->where( 'friend_id', $request->friend_id )
                  ->orWhere( 'user_id', $request->friend_id )->where( 'friend_id', $authUser->id );
            } )
            ->exists();

        if ( !$isFriend ) {
            return response()->json( [
                'message' => __( 'user.not_a_friend' ),
                'errors'  => [ 'friend_id' => __( 'user.not_a_friend' ) ],
            ], 422 );
        }
        ...
        if ( !$recipient ) {
            return response()->json( [
                'message' => __( 'user.friend_not_found' ),
            ], 404 );
        }
        ...
        try {
            ...
            $userVoucher->user_id             = $recipient->id;
            $userVoucher->gifted_from_user_id = $authUser->id;
            $userVoucher->redeem_from         = 3;
            $userVoucher->secret_code         = strtoupper( \Str::random( 8 ) );
            $userVoucher->save();
        ...
        self::sendNotification( $recipient, 'voucher', __( 'notification.voucher_gifted_content', [ 'name' => $authUser->fullname ] ) );

        UserService::createUserNotification(
            $recipient->id,
            'notification.voucher_gifted_title',
            'notification.voucher_gifted_content',
            null,
            'voucher_gifted'
        );

        return response()->json( [
            'message'      => __( 'voucher.voucher_gifted' ),
            'message_key'  => 'voucher_gifted',
        ] );
    }
