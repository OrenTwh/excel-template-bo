...
class ProjectServicev2
{
    ...
    public static function createProject( $request ) {
        ...
        $rules = [
            ...
            'new_floorplans.*' => 'required|file|mimes:jpeg,png,jpg,gif,pdf|max:5120',
    ...
    public static function updateProject( $request ) {
        ...
        $rules = [
            ...
            // File rules for floorplans
            'new_floorplans.*' => 'nullable|file|mimes:jpeg,png,jpg,gif,pdf|max:5120',
        ...
        try {
            ...
            $projects->getCollection()->transform(function ($project) use ($perPage, $request) {
                ...
                $buildupText = $minBuildup && $maxBuildup
                    ? ($minBuildup == $maxBuildup ? number_format($minBuildup) . ' sqft' : 'From ' . number_format($minBuildup) . ' sqft - ' . number_format($maxBuildup) . ' sqft')
                    : null;
                ...
                $pricePsfText = $propertyRanges->min_price_psf && $propertyRanges->max_price_psf
                    ? ($propertyRanges->min_price_psf == $propertyRanges->max_price_psf
                        ? '' . number_format($propertyRanges->min_price_psf)
                        : '' . number_format($propertyRanges->min_price_psf) . ' - ' . number_format($propertyRanges->max_price_psf))
                    : null;
                ...
                $priceUnitText = $propertyRanges->min_price_unit && $propertyRanges->max_price_unit
                    ? ($propertyRanges->min_price_unit == $propertyRanges->max_price_unit
                        ? '' . number_format($propertyRanges->min_price_unit)
                        : '' . number_format($propertyRanges->min_price_unit) . ' - ' . number_format($propertyRanges->max_price_unit))
                    : null;
                ...
                $maintenanceText = $propertyRanges->min_maintenance && $propertyRanges->max_maintenance
                    ? ($propertyRanges->min_maintenance == $propertyRanges->max_maintenance
                        ? '' . number_format($propertyRanges->min_maintenance)
                        : '' . number_format($propertyRanges->min_maintenance) . ' - ' . number_format($propertyRanges->max_maintenance))
                    : null;
                ...
                $baseData = [
                    ...
                    'floorplans_count' => $project->floorplans
                    ->filter(function ($floorplan) {
                        return $floorplan->property !== null; // only keep floorplans with property
                    })->count()
                ];
        ...
        try {
            ...
            if ($propertyRanges && $propertyRanges->min_price_psf && $propertyRanges->max_price_psf) {
                if ($propertyRanges->min_price_psf == $propertyRanges->max_price_psf) {
                    $pricePsfText = '' . number_format($propertyRanges->min_price_psf);
                } else {
                    $pricePsfText = '' . number_format($propertyRanges->min_price_psf) . ' - ' . number_format($propertyRanges->max_price_psf);
                }
            }
            ...
            if ($propertyRanges && $propertyRanges->min_price_unit && $propertyRanges->max_price_unit) {
                if ($propertyRanges->min_price_unit == $propertyRanges->max_price_unit) {
                    $priceUnitText = '' . number_format($propertyRanges->min_price_unit);
                } else {
                    $priceUnitText = '' . number_format($propertyRanges->min_price_unit) . ' - ' . number_format($propertyRanges->max_price_unit);
                }
            }
            ...
            if ($propertyRanges && $propertyRanges->min_maintenance && $propertyRanges->max_maintenance) {
                if ($propertyRanges->min_maintenance == $propertyRanges->max_maintenance) {
                    $maintenanceText = '' . number_format($propertyRanges->min_maintenance);
                } else {
                    $maintenanceText = '' . number_format($propertyRanges->min_maintenance) . ' - ' . number_format($propertyRanges->max_maintenance);
                }
            }
            ...
            $projectData = [
                ...
                'floorplans' => $project->floorplans
                    ->filter(function ($floorplan) use ($request) {
                        return $floorplan->property !== null; // only keep floorplans with property
                    })->map(function ($floorplan) use ($request) {
                    return [
                        'property_id' => $floorplan->property ? $floorplan->property->id : 0,
                        'translations' => $floorplan->property->decoded_translations,
                        'sequence' => $floorplan->sequence,
                        'image' => $floorplan->image_path,
                        'image_path' => $floorplan->image_path,
                        'galleries' => $floorplan->property->galleries()
                        ->orderBy('sequence')
                        ->get()
                        ->map(function ( $gallery ) {
                            return [
                        ...
                        'available_units' => $floorplan?->property ?
                            self::getUnitsByType($floorplan->property, $request->unit_type, 'available')
                                ->orderBy('floor', 'asc')
                                ->orderBy('unit_number', 'asc')
                                ->get()
                                ->map(function ($unit) use ($floorplan) {
                            return [
                                'id' => $unit->id,
                                'property_id' => $unit->property_id,
                                'translations' => $floorplan->property ? $floorplan->property->decoded_translations : $floorplan->project->decoded_translations,
                                'property_name' => $floorplan->property ? $floorplan->property->name : null,
            ...
            $results = $projects->select([
                ...
                ->map(function ($project) {
                    ...
                    return [
                        ...
                        'logo_url' => $project->logo ? asset('storage/' . $project->logo) : null,
     ...
     */
    private static function getUnitsByType($property, $unitType = null, $unitStatus = null)
    {
        switch($unitType) {
            case 1:
            ...
            case 4:
            default:
                $query = $property->units(); // Default units relationship (includes unit_type 4 and null)
                break;
        }
