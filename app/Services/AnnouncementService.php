...
class AnnouncementService
{
    ...
    public static function updateAnnouncement( $request ) {
        ...
        try {
            ...
            // Parse translations from JSON
            $updateAnnouncement->translations = ( $request->translations );
    ...
    private static function filter( $request, $model ) {
        ...
        if ( !empty( $request->created_date ) ) {
            if ( str_contains( $request->created_date, 'to' ) ) {
                ...
                $model->whereBetween( 'announcements.created_at', [ date( 'Y-m-d H:i:s', $start->timestamp ), date( 'Y-m-d H:i:s', $end->timestamp ) ] );
            } else {
        ...
        if ( !empty( $request->title ) ) {
            $model->where(function ($q) use ($request) {
                $q->where('announcements.translations', 'LIKE', '%' . $request->title . '%')
                  ->orWhere('announcements.title', 'LIKE', '%' . $request->title . '%');
            });
    ...
    public static function removeAnnouncementGalleryImage( $request ) {
        ...
        return response()->json( [
            'message' => __( 'template.x_updated', [ 'title' => Str::singular( __( 'announcement.image' ) ) ] ),
        ] );
    }
    ...
    public static function getAnnouncements( $request )
    {
        ...
        $announcements = $announcements->map(function ($announcement) use ( $claimedAnnouncementIds ) {
            ...
            $announcement->append([ 'image_path', 'decoded_translations' ]);
    ...
    /**
     * API: Search announcements with filtering and pagination
     */
    public static function searchAnnouncementsApi($request)
    {
        $validator = Validator::make($request->all(), [
            'search' => 'nullable|string|max:255',
            'show_claimed' => 'nullable|integer|in:0,1',
            'discount_type' => 'nullable|integer|in:1,2,3',
        ...
        try {
            ...
            $query = Announcement::with(['voucher'])
                ->where('status', 10)
                ->where(function ($query) {
                    $query->where(function ($query) {
                        $query->whereNull('start_date');
            ...
            if ($request->filled('search')) {
                ...
                $query->where(function ($q) use ($search) {
                    $q->whereRaw('LOWER(title) LIKE ?', ["%{$search}%"])
                          ->orWhereRaw('LOWER(translations) LIKE ?', ["%{$search}%"]);
                });
            }
            ...
            // Filter by status (override default status filter if provided)
            if ($request->filled('status')) {
                $query->where('status', $request->status);
            }

            // Handle claimed/unclaimed filtering
            if ($request->show_claimed != 1) {
                // Exclude announcements already viewed if `view_once` is enabled
                $query->whereNotIn('id', function ($subQuery) use ($user) {
                    $subQuery->select('announcement_id')
            ...
            // Get claimed announcement IDs for current user
            $claimedAnnouncementIds = AnnouncementReward::where('user_id', $user->id)
                ->pluck('announcement_id')
                ->toArray();

            // Transform the data for API response
            $announcements->getCollection()->transform(function ($announcement) use ($claimedAnnouncementIds) {
                $announcementTitle = $announcement->title;
                $announcementDescription = $announcement->description;

                if($announcement->translations) {
                    $decoded = $announcement->decoded_translations;
                    $announcementTitle = $decoded ? $decoded->en->title : $announcement->title;
                    $announcementDescription = $decoded ? $decoded->en->description : $announcement->description;
                }
                ...
                return [
                    ...
                    'translations' => $announcement->decoded_translations,
                    'image' => $announcement->image_path,
                    'unclaimed_image' => $announcement->unclaimed_image_path,
                    'claiming_image' => $announcement->claiming_image_path,
                    'claimed_image' => $announcement->claimed_image_path,
                    'discount_type' => $announcement->discount_type,
                    'discount_type_label' => $announcement->discount_type_label,
                    'promo_code' => $announcement->promo_code,
                    'start_date' => $announcement->start_date,
                    'expired_date' => $announcement->expired_date,
                    'view_once' => $announcement->view_once,
                    'new_user_only' => $announcement->new_user_only,
                    'status' => $announcement->status,
                    'claimed' => in_array($announcement->id, $claimedAnnouncementIds) ? 'claimed' : 'unclaim',
                    'voucher' => $announcement->voucher ? [
                        'id' => $announcement->voucher->id,
                        'title' => $announcement->voucher->title,
                        'discount_type' => $announcement->voucher->discount_type,
                        'type' => $announcement->voucher->type,
                    ] : null,
            ...
            return response()->json([
                'success' => false,
                'message' => 'An error occurred while searching announcements',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    /**
     * API: Get single announcement details
     */
    public static function getAnnouncementApi($request)
    {
        $validator = Validator::make($request->all(), [
        ...
        try {
            ...
            $announcement = Announcement::with(['voucher'])
                ->where('id', $request->id)
                ->where('status', 10)
                ->first();
            ...
            // Check if user is eligible to view this announcement
            if ($announcement->new_user_only && $user->created_at->diffInDays(now()) > 7) {
                return response()->json([
                    'success' => false,
                    'message' => 'This announcement is only available for new users'
                ], 403);
            }

            // Check if announcement is within date range
            $now = now();
            if (($announcement->start_date && $now < $announcement->start_date) ||
                ($announcement->expired_date && $now > $announcement->expired_date)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Announcement is not currently available'
                ], 403);
            }
            ...
            // Check if user has claimed this announcement
            $isUserClaimed = AnnouncementReward::where('user_id', $user->id)
            ...
            $announcementData = [
                ...
                'discount_type' => $announcement->discount_type,
                'discount_type_label' => $announcement->discount_type_label,
                'discount_amount' => $announcement->discount_amount,
                'promo_code' => $announcement->promo_code,
                'min_spend' => $announcement->min_spend,
                'min_order' => $announcement->min_order,
                'start_date' => $announcement->start_date,
                'expired_date' => $announcement->expired_date,
                'expired_in' => $announcement->expired_in,
                'view_once' => $announcement->view_once,
                'new_user_only' => $announcement->new_user_only,
                'status' => $announcement->status,
                'claimed' => $isUserClaimed ? 'claimed' : 'unclaim',
                'voucher' => $announcement->voucher ? [
                    'id' => $announcement->voucher->id,
                    'title' => $announcement->voucher->title,
                    'description' => $announcement->voucher->description,
                    'discount_type' => $announcement->voucher->discount_type,
                    'type' => $announcement->voucher->type,
                    'promo_code' => $announcement->voucher->promo_code,
                    'total_claimable' => $announcement->voucher->total_claimable,
                    'points_required' => $announcement->voucher->points_required,
                    'start_date' => $announcement->voucher->start_date,
                    'expired_date' => $announcement->voucher->expired_date,
                    'validity_days' => $announcement->voucher->validity_days,
                ] : null,
                'created_at' => $announcement->created_at,
                'updated_at' => $announcement->updated_at,
            ];
